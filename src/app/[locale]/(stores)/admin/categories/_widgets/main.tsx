...
const Main = () => {
  ...
  const { data: categoriesRaw, isLoading, refetch } = useFetchData(
    'admin-categories',
    CategoryServices.FetchAll() as unknown as IGeneric,
  )
  ...
  const openCreate = () => { setSelectedCategory(null); setSheetOpen(true) }
  const openEdit = (row: FlatCategory) => { setSelectedCategory(row); setSheetOpen(true) }
  const closeSheet = () => { setSheetOpen(false); setSelectedCategory(null) }
  const handleFormSuccess = () => { closeSheet(); refetch() }
      ...
      cellRenderer: ({ data }: any) => {
        ...
        return (
          <div style={{ paddingLeft: `${data.depth * 24}px` }} className="flex items-center gap-1.5 h-full">
            ...
            <span className={`text-sm ${data.depth === 0 ? 'font-semibold text-stone-800' : 'text-stone-600'}`}>
              {data.name}
            </span>
          </div>
        )
      ...
      <SheetTemplate
        ...
        content={
          <CategoryForm
            ...
            parentOptions={
              selectedCategory
                ? topLevelOptions.filter(o => o.id !== selectedCategory.id)
                : topLevelOptions
            }
