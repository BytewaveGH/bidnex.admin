...
import {
  CheckCircle, XCircle, Users, UserX, Wallet, Search, Filter, X,
  ...
  return (
    <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 bg-gray-50/50 rounded-b-xl">
      <span className="text-xs text-gray-500">
        {total > 0 ? `Showing ${from}–${to} of ${total} users` : 'No users found'}
      </span>
...
const VendorLotsContent = ({ vendor }: { vendor: IAdminUser }) => {
  ...
  const { data: lotsRaw, isLoading } = useFetchData(
    `vendor-lots-${vendor.id}`,
    LotServices.FetchByVendor(vendor.id) as unknown as IGeneric,
  )
  const allLots = (lotsRaw as ILot[]) ?? []

  const lots = reviewFilter === 'all'
    ? allLots
    : allLots.filter(l => l.reviewStatus === reviewFilter)
    ...
    <div className="flex flex-col gap-4 p-5 flex-1 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto flex flex-col gap-2 pr-1">
        {lots.length === 0 ? (
          ...
          lots.map(lot => (
            <div key={lot.id} className="bg-white border border-gray-100 rounded-2xl p-3.5 flex items-start gap-3 hover:shadow-sm transition-shadow">
              {/* Image */}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start gap-2 flex-wrap">
                  ...
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold capitalize flex-shrink-0 ${REVIEW_STATUS_STYLES[lot.reviewStatus] ?? 'bg-gray-100 text-gray-500'}`}>
                    {lot.reviewStatus}
                  </span>
                </div>
...
const Main = () => {
  ...
  const handleCreditWallet = async () => {
    ...
    setIsActioning(true)
    try {
      await request(UserAdminServices.CreditWallet(creditTarget.id, {
      ...
      refetch()
    } catch { toast.error('Failed to credit wallet') } finally { setIsActioning(false) }
  }
  ...
  const columns: ColDef[] = useMemo(() => [
    ...
    },
    {
      ...
      cellRenderer: ({ value }: any) => (
        <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold capitalize ${TYPE_STYLES[value as UserAccountType] ?? 'bg-gray-100 text-gray-600'}`}>
          {value}
        </span>
      ),
    },
    {
      ...
      cellRenderer: ({ value }: any) => value
        ? <div className="flex items-center justify-center h-full"><CheckCircle className="h-4 w-4 text-emerald-500" /></div>
        : <div className="flex items-center justify-center h-full"><XCircle className="h-4 w-4 text-gray-300" /></div>,
    },
    {
      field: 'status',
      headerName: 'Status',
    ...
    },
    {
      ...
      width: 120,
      valueFormatter: (p: any) => p.value
        ? new Date(p.value).toLocaleDateString(undefined, { day: 'numeric', month: 'short', year: 'numeric' })
        : '—',
            ...
            <div className="bg-gray-50 rounded-xl px-4 py-3 flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-endeavour to-veniceBlue flex items-center justify-center">
                <span className="text-sm font-bold text-white">
                  {creditTarget.username[0]?.toUpperCase() ?? 'U'}
                </span>
              </div>
              <div>
