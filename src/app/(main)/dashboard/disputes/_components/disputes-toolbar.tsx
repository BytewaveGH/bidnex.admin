...
import { DisputePriorityFilter } from "./dispute-priority-filter";
import { DisputeStatusFilter } from "./dispute-status-filter";

interface DisputesToolbarProps<TData> {
  table: Table<TData>;
}

export function DisputesToolbar<TData>({ table }: DisputesToolbarProps<TData>) {
  ...
  return (
    <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
      <div className="flex flex-1 flex-wrap items-center gap-2">
        ...
        />
        <DisputeStatusFilter table={table} />
        <DisputePriorityFilter table={table} />
        {isFiltered && (
          <Button
            variant="destructive"
            onClick={() => {
