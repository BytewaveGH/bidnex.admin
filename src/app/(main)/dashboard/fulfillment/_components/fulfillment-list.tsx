...
import { type FulfillmentOrderSummary, orderStatuses } from "./fulfillment-data";
...
interface ApiOrdersResponse {
  data?: {
    ...
    page?: number;
    limit?: number;
    data?: FulfillmentOrderSummary[];
  };
  status?: boolean;
}

function normalise(res: ApiOrdersResponse): { orders: FulfillmentOrderSummary[]; total: number } {
  const inner = res.data;
  const orders = inner?.data ?? [];
  const total = inner?.count ?? orders.length;
  return { orders, total };
}
...
export function FulfillmentList() {
  ...
  const {
    ...
    queryKey: ["admin-fulfillment-orders", page, pageSize, search, status],
    queryFn: () => {
      const svc = FulfillmentServices.FetchAll({
        page: page + 1,
        limit: pageSize,
        ...(search ? { search } : {}),
        ...(status !== "all" ? { status } : {}),
      });
      return apiRequest<ApiOrdersResponse>(svc.endpoint, token, { params: svc.params });
    },
    enabled: sessionStatus === "authenticated",
    placeholderData: (prev) => prev,
  });

  const { orders, total } = React.useMemo(() => normalise(raw ?? {}), [raw]);
    ...
    <section>
      <Card>
        <CardHeader>
          ...
          <CardDescription>Track each order from escrow through delivery and seller payout.</CardDescription>
                ...
                {isLoading ? (
                  ...
                  table.getRowModel().rows.map((row) => (
                    <TableRow
                      ...
                      onClick={() => router.push(`/dashboard/fulfillment/${row.original.lotId}`)}
                    >
